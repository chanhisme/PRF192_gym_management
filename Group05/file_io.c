...
void loadBinaryFile(){
	...
	if(!fptr){
		...
		//số 1 là record tức 1 record có cấu trúc khi 1 struct member
		while(fread(&tmp, sizeof(struct memberProfile), 1, fptr)){
            //đọc giá trị nhị phân từ file đưa vào ram, 
            //từ ram đưa lên chương trình dạng text
			time_t rawTime = tmp.registerTime;
            //dịch sang định dạng ngày tháng năm;
...
void autoLoadFile(struct memberProfile **members, int *total,
    ...
    if (!fptr){
		...
		while(fgets(line, sizeof(line), fptr)){
            ...
            //nó copy từ vị trí 6 tới hết thoi
                ...
                //1 loạt phép tính đổi từ dạng ngày tháng năm về số giây
                //struct tm (ngày/tháng/năm) → time_t (số giây)
            ...
            //kiểm tra có tồn tại thì sẽ cộng
                ...
                //nếu member đó tồn tại trainer
                if(temp.trainerId[0] != '\0'){
                	//thì tìm trainer rồi tăng biến memberCnt của trainer lên
...
//snprintf nối chuỗi finalName vào .txt
