...
void removeMember(struct memberProfile ** members, int * total){
    ...
    if(index != -1 && canRemove(*members, index) != 0){
        ...
        //nếu trong mảng còn 1 phần tử thì mình free mảng đó luôn
        if( *total - 1 == 0){
            free(*members);
            *members = NULL;
        }
...
void displayAllMember(int total, struct memberProfile*members){
    if(total == 0 || members == NULL){
        ...
        printf("\n===== MEMBER LIST =====\n");
        printf("%-12s | %-12s | %-18s | %-6s | %-12s | %-10s | %-6s\n",
        "ID", "Trainer", "Name", "Birth", "Type", "Date", "Active");
        for(int i = 0; i < total; i++){
        struct tm *t = localtime(&members[i].registerTime);
        printf("%-12s | %-12s | %-18s | %-6d | %-12s | %02d/%02d/%04d | %-6d\n",
            members[i].memberId,
            members[i].trainerId,
            members[i].fullName,
            members[i].birthYear,
...
void displayMember(int i, struct memberProfile * members){
    struct tm *t = localtime(&members[i].registerTime);
    printf("%-10s | %-10s | %-15s | %-6s | %-10s | %-10s | %-6s\n",
                "ID", "Trainer", "Name", "Birth", "Type", "Date", "Active");
